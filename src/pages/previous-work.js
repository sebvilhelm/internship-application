  ...
  border-color: ${props =>
    props.type === 'wordpress'
      ...
      : props.type === 'javascript'
        ? theme.tertiary
        : props.type === 'nodejs' ? theme.green : theme.primary};
`;
...
class PreviousWork extends React.Component {
  ...
  render() {
    const items = this.state.filter
      ? portfolio.filter(item => item.type === this.state.filter)
      : portfolio;
    const types = [
        ...
        <section>
            ...
            meta={[
              {
                name: 'description',
                content:
                  "Here is a selection of stuff that I've worked on, and that I can show",
              },
            ]}
          />
          ...
          <GridContainer>
            <Card>
              ...
              <TextIntro>
                This is a selection of some of the things I've worked on.
                Unfortunately I'm not able to share code from my previous job,
                so this is mostly a showcase of my work experience. I haven't
                been able to do any public React work, so let's talk, so I can
                showcase that I actually know that stuff too{' '}
                <Psst>
                  (This site is built on React and GatsbyJS, you know)
                </Psst>
              </TextIntro>
              ...
              <TagList>
                {types.map(type => (
                  <TagItem
                    active={type.filter === this.state.filter}
                    key={type.filter}
                  >
                    <Button
                      type="small"
                      onClick={() => this.filterList(type.filter)}
                    >
                      {type.name}
                    </Button>
                  </TagItem>
            ...
            {items.map(({ type, link, text, title, linkText, linkType }) => {
              ...
              return (
                <Card showBorder="true" type={type} key={title}>
                  <CardTitleWrapper>
                    <UpperTitleDescription type={type}>
                      {type}
                    </UpperTitleDescription>
